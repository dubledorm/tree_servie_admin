...
RSpec.describe Api::NodesController, type: :request do
  ...
  describe 'all_children#' do
    ...
    it 'should return empty list' do
      get(all_children_api_instance_tree_node_path(instance_id: instance1, tree_id: tree1, id: node121))
      expect(response).to have_http_status(200)
      expect(JSON.parse(response.body)).to eq([])
    end

    it 'should return 4 nodes' do
      get(all_children_api_instance_tree_node_path(instance_id: instance1, tree_id: tree1, id: node1))
      expect(response).to have_http_status(200)
      expect(JSON.parse(response.body).count).to eq(4)
    end

    it_should_behave_like 'get response 404' do
      subject { get(all_children_api_instance_tree_node_path(instance_id: instance1, tree_id: tree1, id: node2)) }
    end

    it 'should return  node' do
      get(all_children_api_instance_tree_node_path(instance_id: instance1, tree_id: tree2, id: node2))
      expect(response).to have_http_status(200)
      expect(JSON.parse(response.body).count).to eq(0)
    end
  end
